    const skrybaApp = {
            currentFileId: null,
            open: (fileItem) => { skrybaApp.currentFileId = fileItem ? fileItem.id : null; document.getElementById('skryba-editor').innerHTML = fileItem ? (fileItem.content || '') : ''; document.getElementById('skryba-filename-display').innerText = fileItem ? fileItem.name : 'Nowy Plik.txt'; winManager.open('skryba'); },
            newFile: () => { skrybaApp.currentFileId = null; document.getElementById('skryba-editor').innerHTML = ''; document.getElementById('skryba-filename-display').innerText = 'Nowy Plik.txt'; },
            ...
            promptSaveToSystem: () => {
                ui.showPrompt("Zapisz plik jako:", document.getElementById('skryba-filename-display').innerText, "Zapisz", (name) => {
                    if(!name) name = "Nowy Plik.txt"; if(!name.endsWith('.txt')) name += '.txt';
